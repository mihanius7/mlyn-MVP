...
public class InteractionProcessor implements OneTimePerStepProcessable {
	...
	public void setBeta(double r1, double r2, double e1, double e2, double u1, double u2) {
		...
		MainWindow.println(String.format(Lang.RECOIL_BY_HERTZ + ", beta = %.3e", beta));
	}
	...
	public void setUseExternalForces(boolean b) {
		useExternalForces = b;
		MainWindow.println(Lang.EXTERNAL_FORCES + ": " + useExternalForces);
	}
	...
	public void setUsePPCollisions(boolean b) {
		...
		recalculateNeighborsNeeded();
		MainWindow.println(Lang.COLLISIONS_PP + ": " + this.usePPCollisions);
	}

	public boolean isUseSPCollisionsNeeded() {
		boolean b = false;
		System.out.println(Lang.SPRINGS_NUMBER + ": " + springs.size());
		...
		MainWindow.println(Lang.COLLISIONS_PS_NEEDED + ": " + b);
		return b;
	}
	...
	public void message() {
		MainWindow.println(
				String.format(Lang.MAX_INTERACTION_DEFINING_DISTANCE + ": %.1e м", maxPairInteractionDistance));
	}
