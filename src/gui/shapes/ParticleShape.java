...
public class ParticleShape extends Shape {
	...
	public void paintShape(Graphics2D targetG2d, Viewport viewport) {
		...
		if (viewport.isDrawTracks() && viewport.isScaled()) {
			viewport.tracksFrame.setColor(color);
			viewport.tracksFrame.drawLine(lastPaintedX, lastPaintedY, x, y);
		}
		lastPaintedX = x;
		lastPaintedY = y;
		...
		if (drawTags || p.isSelected()) {
			...
			y += viewport.labelsFont.getSize() * stringsInterval;
			targetG2d.drawString(String.format("%.1e m/s", p.defineVelocity()), x, y);
		}
	}
